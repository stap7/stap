...
<li> Click a blue circle to remove it from the circuit.</ul>"""})
	...
	send({"Timer":SECONDS_PER_TRIAL})
	#stap.setNumSpecs(Timer = stap.numSpec(animate=[-1,1,0]))
	#stap.setAutoAction({submitSolutionBtn:3},wait=SECONDS_PER_TRIAL,waitID='submitTimer')
	chosenLocs=[]
	points=[]
	action=recv()
